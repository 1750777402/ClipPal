                    ...
                    app_handle
                        ...
                        .save_file(move |file_path| {
                            ...
                            if let Some(select_path) =
                                file_path.and_then(|p| p.as_path().map(|p| p.to_path_buf()))
                            {
...
pub(crate) async fn copy_record_to_clipboard(
    state: &AppContext,
    param: &CopyClipRecord,
) -> Result<(), String> {
    let rb: &RBatis = state.db();
...
pub(crate) async fn create_temp_files_with_correct_names(
...
pub(crate) fn show_accessibility_permission_dialog(app_handle: &AppHandle) {
