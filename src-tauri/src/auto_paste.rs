...
use windows::{
    Win32::{
        ...
        UI::{
            WindowsAndMessaging::{
                GetForegroundWindow, SetForegroundWindow, GetWindowTextW,
...
use cocoa::{
    ...
    base::{id, nil, NO, YES},
    foundation::{NSArray, NSString, NSNumber},
};
#[cfg(target_os = "macos")]
use core_graphics::{
    event::{CGEvent, CGEventType, CGKeyCode, CGEventFlags},
    event_source::{CGEventSource, CGEventSourceStateID},
    window::{kCGWindowOwnerPID, kCGWindowNumber, CGWindowListOption, CGWindowListCopyWindowInfo},
};
#[cfg(target_os = "macos")]
use objc::{msg_send, sel, sel_impl, runtime::Object};

#[cfg(any(windows, target_os = "macos"))]
static PREVIOUS_WINDOW: Lazy<Arc<Mutex<Option<WindowInfo>>>> = Lazy::new(|| {
...
pub fn save_foreground_window() {
    ...
    if let Ok(mut previous) = PREVIOUS_WINDOW.lock() {
        *previous = Some(window_info.clone());
        log::info!("保存焦点窗口: {} (PID: {})", window_info.title, window_info.process_id);
    }
}
        ...
        for i in 0..window_count {
            ...
            if pid_obj != nil {
                ...
                if window_pid == frontmost_app_pid {
                    ...
                    let window_number_obj: id = msg_send![window_info, objectForKey: window_number_key];
...
pub fn auto_paste_to_previous_window() -> Result<(), String> {
    let window_info = {
        let previous = PREVIOUS_WINDOW.lock()
...
pub fn auto_paste_to_previous_window() -> Result<(), String> {
    ...
    log::info!("尝试自动粘贴到窗口: {} (PID: {})", window_info.title, window_info.process_id);
...
fn send_ctrl_v_windows() -> Result<(), String> {
    ...
    if result != inputs.len() as u32 {
        return Err(format!("发送按键失败，期望发送 {} 个事件，实际发送 {} 个", inputs.len(), result));
    }
